	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const searchContext = results.length > 0
				...
				).join('\n\n')}\n\nИнструкции: Основываясь на результатах поиска выше, пожалуйста, предоставьте ответ на запрос пользователя. При ссылке на информацию указывайте номер источника в скобках, например [1], [2] и т.д. Используйте простой русский язык и простые слова. Самое важное: перед тем как дать окончательный ответ, подумайте вслух, думайте шаг за шагом. Думайте глубоко и просмотрите свои шаги, сделайте 3-5 шагов размышлений. Оберните размышления в теги <think>. Начните с <think> и закончите </think>, а затем дайте окончательный ответ.`
				: '';
	...
	return (
		<>
            ...
            font-medium shadow-sm hover:shadow-md hover:-translate-y-0.5"
					>
						<span className="text-sm">Попробовать Exa API</span>
					</a>
					<div className="flex items-center gap-4 text-md text-gray-600">
						...
						>
							<span className="underline">Больше демо</span>
						</a>
						...
						>
							<span className="underline">Исходный код проекта</span>
							<svg
				...
				<div className="space-y-6">
					{messages.filter(m => m.role !== 'system').map((message) => (
						<div key={message.id}>
							...
							>
								<div
									className={`rounded py-3 max-w-[85%] ${message.role === 'user'
														...
														{(thinking || !isComplete) && (
															<div className="mb-10 space-y-4">
																<div className="flex items-center gap-2">
																	...
																	>
																		<svg
																			className={`w-5 h-5 transform hover:text-[var(--brand-default)] transition-transform ${isThinkingExpanded ? 'rotate-0' : '-rotate-180'}`}
																			fill="none"
																			viewBox="0 0 24 24"
																			stroke="currentColor"
																		>
																		...
																		<h3 className="text-md font-medium">Рассуждения</h3>
																	</button>
																</div>
																{isThinkingExpanded && (
							...
							{message.role === 'user' && !isSearching && searchResults.length > 0 && (
								<div className="my-10 space-y-4">
									...
									<div className="flex items-center gap-2">
										...
										>
											<svg
												className={`w-5 h-5 transform hover:text-[var(--brand-default)] transition-transform ${isSourcesExpanded ? 'rotate-0' : '-rotate-180'}`}
												fill="none"
												viewBox="0 0 24 24"
												stroke="currentColor"
											>
											...
											<h3 className="text-md font-medium">Результаты поиска</h3>
										</button>
									</div>
